...
- [blake playground](https://github.com/play-grounds/blake) - The same wallet as blaketest switched to mainnet. [Live](https://play-grounds.github.io/blake/). *Experimental and unaudited, keys stored unencrypted in the browser, small amounts only.*
...
- [Kilombino Bitcoin-Blake2b wallet](https://github.com/Kilombino/pyblock-watch) - Android wallet that watches any xpub across both chains side by side, or spends as a hot wallet whose seed is generated on the phone from physical dice and held behind Android Keystore biometrics. Apache-2.0, reproducible builds, no Google Play Services, and the BLAKE2b side can point at your own Electrum server. Published through [Zapstore](https://zapstore.dev/). *Spends are P2WPKH signed with plain `SIGHASH_ALL`, not the unified sighash, so spending a coin that predates the fork can be replayed onto the SHA256d chain. Watch-only mode holds no keys and has no such caveat.*
...
- [Tyger Pool](https://tygerpool.com/) - DATUM-only, non-custodial, paid to your address from the coinbase. 0% launch fee with a promise of 30 days' notice before it changes. One-click setup via its [Umbrel store](https://github.com/tygerpool/umbrel-store); stratum is paused. *New, site still being reworked, pool server source not published.*
